          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                previous = this.traverse(this.heads, Math.max(1, this.heads.length));
                prevPlusHeads = Object.assign({}, this._headsIndex, previous);
                sortedEntries = Object.values(prevPlusHeads).sort(this.sortFn);
                rawNexts = sortedEntries.map(getNextPointers).reduce(flatMap, []); // Trim to maxHeads

                while (rawNexts.length > pointerCount - 1) {
                  rawNexts.shift();
                }

                hashes = sortedEntries.map(getHash);
                rawNexts.unshift(hashes);
                nexts = rawNexts.reduce(flatMap, []); // To check against the previous calculation
                // pointerCount = 1
